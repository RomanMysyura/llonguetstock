    ...
    try {
        ...
        movements.value.forEach(movement => {
            ...
            if (!groupedMovements.value[dateKey][movement.name]) {
                groupedMovements.value[dateKey][movement.name] = {
...
<template>
    <div class="w-full bg-white p-2">
        ...
        <div class="overflow-y-auto h-[calc(95vh-4rem)]">
            <!-- Recorre las fechas en orden descendente -->
            <div v-for="date in Object.keys(groupedMovements).sort((a, b) => new Date(b) - new Date(a))" :key="date"
                class="mb-8 p-2">
                ...
                <table class="table-auto w-full">
                    ...
                    <tbody>
                        <tr v-for="(movement, name, index) in groupedMovements[date]" :key="name"
